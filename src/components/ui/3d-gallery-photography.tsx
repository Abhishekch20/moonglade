...
function GalleryScene({
  ...
  const section = document
    .querySelector("[data-gallery-section]") as HTMLElement | null;
  ...
  return Math.abs(sectionCenter - viewportCenter) < 40; // 👈 tolerance (px)
};
...
const handleWheel = useCallback(
  ...
  lockScrollRef.current = true;
    ...
    // ⬇️ DO NOT always block page scroll
   if (lockScrollRef.current && isSectionCentered()) {
...
useEffect(() => {
  ...
  const onWheel = (e: WheelEvent) => {
    ...
    if (Math.abs(accumulated) > 240) {
      lockScrollRef.current = false; // 🔓 release page scroll
      accumulated = 0;
    }
  };
